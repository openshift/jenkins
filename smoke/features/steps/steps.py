...
def before_feature(context, feature):
    if scenario.name != None and "TEST_NAMESPACE" in scenario.name:
        print("Scenario using env namespace subtitution found: {0}, env: {}".format(
            scenario.name, os.getenv("TEST_NAMESPACE")))
        scenario.name = txt.replace(
...
def jenkinsMasterPodStatus(context):
    ...
    container_status = oc.get_resource_info_by_jsonpath(
        'pods', jenkins_master_pod, current_project, json_path='{.status.containerStatuses[*].ready}')
...
@then(u'we configure custom agents as Kubernetes pod template by creating configmap using "smoke/samples/java-builder-cm.yaml"')
...
def check_build_status(context):
    verify_status(context.current_project, 'build',
                  'pipes-1', 5, 600, 'Complete')
