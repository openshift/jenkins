...
| `.tekton/jenkins-rhel8-multi-arch-push.yaml` | Build PipelineRun triggered on push to `release-rhel8` |
| `.tekton/jenkins-rhel8-multi-arch-pull-request.yaml` | Build PipelineRun triggered on PRs to `release-rhel8` |
...
```bash
IMAGE=$(echo "$SNAPSHOT" | jq -r '.components[] | select(.name == "jenkins-rhel8-multi-arch") | .containerImage')
...
The `.name` selector in the `jq` expression must match the Konflux component name (`jenkins-rhel8-multi-arch`).
...
| PR | Title | Purpose |
|----|-------|---------|
| [#2359](https://github.com/openshift/jenkins/pull/2359) | JKNS-1056: Enable DAST on OpenShift Jenkins | Initial PR — added `.tekton/rapidast-integration-test.yaml` on the `release-rhel8` branch |
...
| [#2361](https://github.com/openshift/jenkins/pull/2361) | JKNS-1071: Draft PR for RHEL8 DAST validation | Throwaway PR to trigger the Konflux pipeline and validate the DAST scan end-to-end (closed, not merged) |

### GitLab MRs (konflux-release-data)

| MR | Title |
|----|-------|
| [!18071](https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/18071) | Enable DAST on OpenShift Jenkins RHEL8 |
...
tenants-config/cluster/stone-prd-rh01/tenants/ocp-tools-jenkins-tenant/
  openshift-jenkins-rhel8/test-scenarios/
...
metadata:
  ...
  name: rapidast-openshift-jenkins-rhel8
  namespace: ocp-tools-jenkins-tenant
spec:
  application: openshift-jenkins-rhel8
  contexts:
    - description: Rapidast integration test for jenkins-rhel8-multi-arch component
      name: component_jenkins-rhel8-multi-arch
  resolverRef:
    params:
      - name: url
...
- **`spec.application`** — the Konflux application name (e.g., `openshift-jenkins-rhel8`)
- **`spec.contexts[].name`** — scopes when the test runs. Must use the format `component_<component-name>` to restrict DAST to a specific component's builds. Using `application` instead would run DAST for every component in the application (including `jenkins-agent-base-rhel8`), which is not desired. The component name is visible in the `appstudio.openshift.io/component` label on PipelineRuns
...
|-------------|-----------|-------------|
| `openshift-jenkins-rhel8` | `jenkins-rhel8-multi-arch` | `component_jenkins-rhel8-multi-arch` |
